...
class XrayCore(BaseProcessManager):
    ...
    """
    def __init__(self, vendor_path: str, config_builder: XrayConfigBuilder, debug_mode: bool = False):
    ...
    def _get_executable_name(self) -> str:
        if sys.platform == "win32": return "xray.exe"
        if sys.platform == "darwin": return "xray_macos"
        return "xray_linux"
    ...
    def _cleanup_config(self) -> None:
        ...
        if self.debug_mode:
            logging.info(f"[DEBUG MODE] Xray temporary config file kept at: {self._config_file_path}")
            return
